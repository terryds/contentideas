...
- [v1.2] trending = its own daily job (trending_run_times, global timezone) instead of a per-run step — owner prefers one predictable daily trending review/digest/draft batch; fetch runs keep tag-drafts only; mid-day threshold crossings wait for the next scheduled job
