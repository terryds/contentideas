...
- **Scheduler** *(reworked 2026-08-20 — per-source cadence)*: scheduling is **per source**, not global. Each source carries its own schedule in one of two modes, plus `max_records` (cap on items fetched per check, default 30), all set when adding and editable inline on the Sources page:
  - **Interval mode** — `check_interval` (15m / 30m / 1h / 3h / 6h / 12h / 24h): due when `last_fetched_at + interval <= now`. A relative countdown anchored to the last check, not the clock.
  - **Clock mode** *(added 2026-08-20)* — `schedule_times`: explicit times of day like `07:00, 19:00`, up to 8, interpreted in the owner's **timezone** (one global `timezone` setting, IANA name, defaulting to the server's zone). Due when a scheduled occurrence has passed since the last fetch — so a boot at 09:00 catches the missed 07:00 slot exactly once; occurrences never queue up. Setting times switches the source to clock mode; clearing them falls back to its interval.
  - One master `Bun.cron` tick fires **every minute** and fetches only due sources; a tick with nothing due creates no run row. `last_fetched_at` is stamped when a source's fetch starts (a crashing source can't hot-loop every minute). "Run now" fetches **all** active sources regardless of due-ness. Timezone math lives in `server/clock.ts` (pure, unit-tested; DST-correct via Intl).
