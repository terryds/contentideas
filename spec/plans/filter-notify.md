...
- **On match:** for YouTube entries, fetch the transcript now (via `transcript.ts`, proxy, retries) — transcript failure does NOT block notification, it's retried when drafting. Then Telegram `sendMessage` (token + chat ID from settings): title, source label, filter's reason, original URL. Delivered → `state=notified`. Send failure → recorded, entry stays `state=new`, resent next run.
